...
class Mai extends Image {
    ...
    constructor(location: Vector2D, zIndex?: number) {
        if (zIndex) {
            super(location, sprite.MAI_SPRITE.clip, zIndex)
        } else {
            super(location, sprite.MAI_SPRITE.clip)
        }
        this.setTex(this.resourceManager.getTex(4))

        this.setAngle(80)

        this.setState(new MaiState.MaiStartState())
